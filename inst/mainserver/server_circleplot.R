...
circosmht <- function(mydata=mytoys,BP= "BP", Chr="Chr", traitsname = c("Trait1_Beta","Trait2_Beta"), logV1 = FALSE, logV2 =FALSE , trait.pvalnam = c("Trait1_P","Trait2_P"), pcut.outlier=0.002){
  ...
  for (i in 1:length(traitidxlist)){
     ...
     circos(R= 120 + (i-1) * 100, cir=db, W= 180, mapping=seg.value, col.v=traitidxlist[i],type="s", B=F, col=tmpcolor,lwd=0.15, scale=T);
     circos(R=100, cir=db, W=100,mapping=outlier.link, type="link",lwd=0.2,col= tmpcolor);
  }
